...
export class OrderController {
    ...
    static async getDetailOrderPage(req: any, res: any) {
        try {
            ...
            if (order.subOrders.length !== 0) {
                for (const item of order.subOrders) {
        ...
        try {
            ...
            if (order.subOrders.length !== 0) {
                ...
                for (const subOrder of subOrders) {
                    let sub = await SubOrder.findOne({_id: subOrder}).populate({path: 'food'});
                    if (sub.quantity !== 0) {
                        arr.push(subOrder)
                    }
                    totalMoney += sub.quantity * sub.food.price
                }
    ...
    static async deleteSubOrder(req: any, res: any) {
        try {
            let subOrderID = req.params.id;
            ...
            if (order) {
                let subOrders = order.subOrders;
                let sub = await SubOrder.findOne({ _id: subOrderID }).populate({path:"food"});
                let multiPrice = sub.quantity * sub.food.price;
                let index = subOrders.findIndex(item => item._id == subOrderID);
                if (index !== -1) {
                    subOrders.splice(index, 1);
                    order.subOrders = subOrders;
                    order.cost -= multiPrice;
                    await order.save()
                    await SubOrder.deleteOne({ _id: subOrderID });
                    return res.json({
                        status: "success",
